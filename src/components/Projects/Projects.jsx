...
function GridItem({ project, late, priority, ref }) {
  const wide = Boolean(project.featured);
  const [lateAtMount] = useState(late);
  const entrance = lateAtMount
...
function Projects() {
  ...
  // Roving tabindex: arrows, Home and End move between tabs and select. The
  // handler sits on each tab (the focusable element) rather than the list.
